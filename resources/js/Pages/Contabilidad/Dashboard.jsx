...
export default function Dashboard({ stats, solicitudesCajaChica = [], solicitudesRegulares = [], ultimosPagos = [], isCajaChica = false, isSoloMayores = false, isMixto = false }) {
    ...
    return (
        <ContabilidadLayout
            title={
                isCajaChica
                    ? "Dashboard Caja Chica (Fralak SRL)"
                    : userIsSoloMayores
                    ? "Dashboard Desembolsos Mayores (Fralak SRL)"
                    : "Dashboard Contable y Desembolsos"
            }
            badgePorPagar={stats.pendientesPagoCount}
        >
            {/* Header Banner */}
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-500/30 mb-3">
                            ...
                            {isCajaChica
                                ? 'Tesorería • Caja Chica Fralak SRL (Hasta 300 BOB)'
                                : userIsSoloMayores
                                ? 'Tesorería • Solicitudes Mayores Fralak SRL (> 300 BOB / USD)'
                                : 'Tesorería Médica • Red Médica Corporativa'}
                        </div>
                        <h2 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">
                            {isCajaChica
                                ? 'Desembolsos y Pagos de Caja Chica'
                                : userIsSoloMayores
                                ? 'Desembolsos Mayores & Compras Hospitalarias'
                        ...
                        <p className="text-sm text-slate-300 mt-1 max-w-2xl">
                            {isCajaChica
                                ...
                                : userIsSoloMayores
                                ? 'Bandeja de auditoría y transferencias bancarias a proveedores para solicitudes mayores a 300 BOB o en USD.'
                    ...
                    <div className="flex flex-wrap items-center gap-3 shrink-0">
                        ...
                        {!isCajaChica && (
                            <Link
                                href={route('contabilidad.solicitudes', userIsSoloMayores ? { estado: 'Aprobado_Jefatura' } : { tipo_monto: 'regular' })}
                            ...
                            >
                                ...
                                <span>{userIsSoloMayores ? 'Por Desembolsar' : 'Regulares'} ({userIsSoloMayores ? stats.pendientesPagoCount : stats.regularesPendientesCount})</span>
                            </Link>
                        )}
                    </div>
                </div>
            ...
            {/* Metric KPI Cards */}
            <div className={`grid grid-cols-1 sm:grid-cols-2 ${(isCajaChica || userIsSoloMayores) ? 'lg:grid-cols-3' : 'lg:grid-cols-4'} gap-5 mb-8`}>
                {/* 1. Caja Chica (<= 300 BOB) - OCULTADO SI ES SOLO MAYORES */}
                {!userIsSoloMayores && (
                    ...
                    <div className="rounded-3xl bg-slate-900 border border-indigo-500/30 p-5 shadow-xl relative overflow-hidden group hover:border-indigo-500/50 transition duration-300">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">
                                {userIsSoloMayores ? 'Por Desembolsar (> 300 BOB / USD)' : 'Regulares / Mayores (> 300 BOB / USD)'}
                            </span>
                        ...
                        <div className="mt-4">
                            ...
                            <p className="text-xs text-slate-400 mt-1">
                                {userIsSoloMayores ? 'Solicitudes mayores aprobadas en espera de pago' : 'Pagos mayores a 300 BOB o USD'}
                            </p>
                        </div>
            ...
            {/* BLOQUE 1: Solicitudes de CAJA CHICA (<= 300 BOB) - OCULTADO SI ES SOLO MAYORES */}
            {!userIsSoloMayores && (
            ...
            {/* BLOQUE 2: Solicitudes REGULARES / MAYORES (> 300 BOB / USD) - Solo visible para Contabilidad General o Solo Mayores */}
            {!isCajaChica && (
                <div className="bg-slate-900 border border-indigo-500/30 rounded-3xl p-6 shadow-xl mb-8">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-lg font-bold text-white tracking-tight flex items-center gap-2">
                                    ...
                                    {userIsSoloMayores
                                        ? 'Solicitudes Pendientes de Desembolso (> 300 BOB o USD)'
                                        : 'Solicitudes Regulares / Mayores (> 300 BOB o USD)'}
                                </h3>
                            </div>
                            <p className="text-xs text-slate-400 mt-0.5">
                                {userIsSoloMayores
                                    ? 'Desembolsos y transferencias bancarias a proveedores aprobadas por jefatura para procesar'
                                    : 'Desembolsos mayores requeridos para contrataciones y pagos principales a proveedores'}
                            </p>
                        </div>

                        <Link
                            href={route('contabilidad.solicitudes', userIsSoloMayores ? { estado: 'Aprobado_Jefatura' } : { estado: 'Aprobado_Jefatura', tipo_monto: 'regular' })}
                        ...
                        >
                            <span>Ver Solicitudes ({stats.regularesPendientesCount})</span>
                            <ArrowUpRight className="w-4 h-4" />
                        </Link>
                    </div>
