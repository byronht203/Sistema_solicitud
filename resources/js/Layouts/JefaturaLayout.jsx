...
export default function JefaturaLayout({ children, title, badgePendientes = 0, badgeMisSolicitudes = 0 }) {
    ...
    const jefeNavigation = [
        { name: 'Dashboard Jefatura', href: route('jefatura.dashboard'), routeName: 'jefatura.dashboard', icon: LayoutDashboard },
        { 
    ...
    const adminNavigation = [
        ...
        { name: 'Módulo Jefatura', href: route('jefatura.dashboard'), routeName: 'jefatura.dashboard', icon: ShieldCheck },
    ...
    const navigation = isAdmin ? adminNavigation : jefeNavigation;
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
            <Head title={title || 'Panel de Jefatura y Aprobaciones'} />
            ...
            >
                ...
                <div className="h-20 flex items-center justify-between px-5 border-b border-slate-800/80 bg-slate-900/90">
                    <Link href={route('jefatura.dashboard')} className="flex items-center gap-3 group">
                ...
                <div className="p-4 border-b border-slate-800/60 bg-slate-950/40">
                    <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-900 border border-slate-800 shadow-sm">
                        <div className="w-10 h-10 rounded-xl bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 flex items-center justify-center font-bold text-lg shrink-0">
                            {user.nombre ? user.nombre.charAt(0) : 'J'}
                        </div>
                        <div className="min-w-0 flex-1">
                            <h4 className="text-xs font-bold text-white truncate">
                                {user.nombre_completo || user.nombre || 'Jefe de Área'}
                            </h4>
                            <p className="text-[11px] text-slate-400 truncate mt-0.5">
                                {user.cargo || 'Jefe / Aprobador'}
                            </p>
                        ...
                        return (
                            <Link
                                ...
                                className={`flex items-center justify-between px-3.5 py-3 rounded-xl font-medium text-sm transition-all duration-200 ${
                                    active
                                        ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white shadow-lg shadow-indigo-600/20 font-semibold'
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h1 className="text-xl font-bold text-white tracking-tight">{title || 'Modulo Jefatura'}</h1>
                                <span className="px-2.5 py-0.5 text-[10px] font-extrabold uppercase bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-md">
                                    Revisión & Aprobación
                                </span>
                            </div>
                            <p className="text-xs text-slate-400 hidden sm:block">
                                Control Operacional y Validación de Gastos Corporativos
                            </p>
                        </div>
                    </div>
